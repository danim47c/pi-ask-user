...
- Per-session Telegram topics (`repository · session-name` or short session ID), durable cross-process routing of free text, idle/steer delivery, inactive historical-topic notices, and safe migration that leaves legacy repository/worktree topics untouched. Topic routing is text-only and requires Telegram private-chat forum topics.
