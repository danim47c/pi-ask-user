...
- Agent idle notifications on Pi's `agent_end` event after the same 60-second grace period. If the user responds, a new turn starts, auto-compaction is in progress, or async subagents are still running, no Telegram message is sent. The idle notification is deferred until compaction and the final async run complete, and subagent child processes never send their own Telegram notifications.
